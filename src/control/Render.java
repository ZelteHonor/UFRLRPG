...
public class Render {
	...
	public void draw(GameObject obj) {
		...
		gc.translate(px, py);// Besoins de monstre pour connaître l'amélioration
								// à faire!

		if (obj instanceof Sword)
			gc.rotate(Math.toDegrees(obj.getAngle())
					+ ((Sword) obj).getAngleDiff());
		else
			gc.rotate(Math.toDegrees(obj.getAngle()));
		...
		gc.drawImage(sprite, -(sprite.getWidth() / 2),
				-(sprite.getHeight() / 2));
		gc.restore();
	}
		...
		for (int j = -1; j <= DH; j++) {
			for (int i = -1; i <= DW; i++) {
				...
				try {
					...
					case WALL:
						gc.drawImage(spriteMap.get("img/wall.png"), px, py);
						break;
					case ROCK:
						gc.drawImage(spriteMap.get("img/stone.png"), px, py);
						break;
					case CAVE:
						gc.drawImage(spriteMap.get("img/dirt.png"), px, py);
						break;
					case DONJON:
						gc.drawImage(spriteMap.get("img/plank.png"), px, py);
						break;
					case TUNNEL:
						gc.drawImage(spriteMap.get("img/plank_alt.png"), px, py);
						break;
					case EXITUP:
						gc.drawImage(spriteMap.get("img/exitup.png"), px, py);
						break;
					case EXITDOWN:
						gc.drawImage(spriteMap.get("img/exitdown.png"), px, py);
						break;
					case BLACK:
					default:
						break;
	...
	public void drawHUD() {
		...
		for (GameObject o : Controller.get().getObjects()) {
			if (o instanceof Monster) {
				...
				if (m.getHealth() != m.MAX_HEALTH) {
					...
					gc.fillRect((m.getX() - Controller.get().getCX())
							* RESOLUTION + 640 - 8, (m.getY() - Controller
							.get().getCY()) * RESOLUTION + 360 - 64,
							16 * (m.getHealth() / (m.MAX_HEALTH * 1.0)), 4);
					gc.setFill(Color.DARKRED);
					gc.fillRect((m.getX() - Controller.get().getCX())
							* RESOLUTION + 641 - 8, (m.getY() - Controller
							.get().getCY()) * RESOLUTION + 361 - 64,
							16 * (m.getHealth() / (m.MAX_HEALTH * 1.0)), 4);

					gc.setFill(Color.DARKGREY);
					gc.fillRect((m.getX() - Controller.get().getCX())
							* RESOLUTION + 640 - 8 + 16
							* (m.getHealth() / (m.MAX_HEALTH * 1.0)),
							(m.getY() - Controller.get().getCY()) * RESOLUTION
									+ 360 - 64,
							16 - 16 * (m.getHealth() / (m.MAX_HEALTH * 1.0)), 4);
		...
		gc.setFill(Color.GREY);
		gc.fillRect(514 + (player.getHealth() / 100.0) * 256.0, 658,
				256 - (player.getHealth() / 100.0) * 256.0, 16);

		gc.setFill(Color.RED);
		gc.fillRect(512, 656, (player.getHealth() / 100.0) * 256.0, 16);
		gc.setFill(Color.DARKGREY);
		gc.fillRect(512 + (player.getHealth() / 100.0) * 256.0, 656,
				256 - (player.getHealth() / 100.0) * 256.0, 16);
		...
		if (player.getWeapon() instanceof Bow)
			gc.drawImage(getSprite("img/crosshair.png"), Controller.get()
					.getInput().getMouse().getSceneX() - 16, Controller.get()
					.getInput().getMouse().getSceneY() - 16);
	...
	private void loadSprites() {
		...
		try {
			Files.walk(Paths.get("img")).forEach(
					filePath -> {
						if (Files.isRegularFile(filePath))
							spriteMap.put("img/"
									+ filePath.getFileName().toString(),
									new Image(filePath.toUri().toString()));
					});
		} catch (IOException e) {
			e.printStackTrace();
		}
		...
		if (percent < 20)
			spriteMap.put("img/deathscreen.png",
					spriteMap.get("img/deathscreen_01.png"));
		else if (percent < 40)
			spriteMap.put("img/deathscreen.png",
					spriteMap.get("img/deathscreen_02.png"));
		else if (percent < 60)
			spriteMap.put("img/deathscreen.png",
					spriteMap.get("img/deathscreen_03.png"));
		else if (percent < 80)
			spriteMap.put("img/deathscreen.png",
					spriteMap.get("img/deathscreen_04.png"));
		else if (percent < 87)
			spriteMap.put("img/deathscreen.png",
					spriteMap.get("img/deathscreen_05.png"));
		else if (percent < 94)
			spriteMap.put("img/deathscreen.png",
					spriteMap.get("img/deathscreen_06.png"));
		else
			spriteMap.put("img/deathscreen.png",
					spriteMap.get("img/deathscreen_07.png"));
	}
}
