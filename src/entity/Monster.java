...
public class Monster extends Entity {
	...
	/**
	 * totalPath Chemin jusqu'au joueur nextNode Prochain objectif pour le
	 * chemin
	 */
	private Node totalPath, nextNode;
	/**
	 * Latence de 1/6 de seconde avant que le monstre ne cherche de nouveau le
	 * joueur au lieu de 1/60 comme le déroulement du jeux.
	 */
	 ...
	 */
	public Monster(double x, double y, int health, int damage, int attackspeed,
			float speed) {
		super(x, y, health);
	...
	public void update(Floor floor) {
		...
		if (seePlayer(floor)) {
			...
			if (tickTillAI == 0) {
				totalPath = pathfinding.Pathfinding.getPath(new Point(
						(int) this.x, (int) this.y), lastTarget, floor);
				tickTillAI = 10;
			}
		...
		if (Math.sqrt(Math.pow(x - floor.getPlayer().getX(), 2)
				+ Math.pow(y - floor.getPlayer().getY(), 2)) < 0.6
				&& cooldown == 0) {
	 ...
	 * 
	 * @return Vrai si le Monstre vois le joueur
	 */
	 ...
	 * 
	 * @return L'angle avec sa cible.
	 */
	private double getMonsTargetAngle() {
		return Math.atan2(nextNode.getCoor().getY() - y + 0.5, nextNode
				.getCoor().getX() - x + 0.5);
	}
