...
public class Render {
	 ...
	 * @param resolution
	 */
	public Render(World.TILE[][] world, Canvas GUI, int DW, int DH,
			int resolution) {
		this.world = world;
	...
	public void clear() {
		...
		GUI.getGraphicsContext2D().fillRect(0, 0, DW * RESOLUTION,
				DH * RESOLUTION);
	...
	public void draw(GameObjects obj) {
		...
		GUI.getGraphicsContext2D().drawImage(sprite, -(sprite.getWidth() / 2),
				-(sprite.getHeight() / 2));
	...
	public void drawShadows() {
		/*
		 * Stop[] stops1 = {new Stop(0.6,Color.TRANSPARENT),new
		 * Stop(1.0,Color.BLACK)}; Stop[] stops2 = {new
		 * Stop(0.0,Color.BLACK),new Stop(1.0,Color.BLACK)};
		 * 
		 * GUI.getGraphicsContext2D().save();
		 * 
		 * //Ombrage général //GUI.getGraphicsContext2D().setGlobalAlpha(0.3);
		 * //GUI.getGraphicsContext2D().fillRect(0, 0, DW*RESOLUTION
		 * ,DH*RESOLUTION); GUI.getGraphicsContext2D().restore();
		 * 
		 * 
		 * //Ombrage radial GUI.getGraphicsContext2D().setFill(new
		 * RadialGradient(30, 0, 0, 0, 1, true, CycleMethod.NO_CYCLE, stops1));
		 * GUI.getGraphicsContext2D().fillOval(0, 0, 1000, 1000);
		 * GUI.getGraphicsContext2D().restore();
		 * GUI.getGraphicsContext2D().setFill(new RadialGradient(0, 0, 0, 0,
		 * 0.7, true, CycleMethod.NO_CYCLE, stops2));
		 * //GUI.getGraphicsContext2D().fillOval(0, 0, 1000, 1000);
		 */
			...
			for (int i = scx; i < (scx + (DW + 2)); i++) {
				...
				try {
					...
					case 0:
						break;
					case 1:
						GUI.getGraphicsContext2D().drawImage(
								spriteMap.get("img/wall.png"),
								(i - scx) * RESOLUTION, (j - scy) * RESOLUTION);
						break;
					case 2:
						GUI.getGraphicsContext2D().drawImage(
								spriteMap.get("img/stone.png"),
								(i - scx) * RESOLUTION, (j - scy) * RESOLUTION);
						break;
					case 3:
						GUI.getGraphicsContext2D().drawImage(
								spriteMap.get("img/dirt.png"),
								(i - scx) * RESOLUTION, (j - scy) * RESOLUTION);
						break;
					case 4:
						GUI.getGraphicsContext2D().drawImage(
								spriteMap.get("img/plank.png"),
								(i - scx) * RESOLUTION, (j - scy) * RESOLUTION);
						break;
					case 5:
						GUI.getGraphicsContext2D().drawImage(
								spriteMap.get("img/plank_alt.png"),
								(i - scx) * RESOLUTION, (j - scy) * RESOLUTION);
						break;
					}
	...
	private class Wall {
		...
		public void recalculate(int x, int y) {
					...
					try {
						...
						if (world[i][j] == TILE.WALL
							...
							if ((world[i][j + 1] == TILE.WALL || world[i][j + 1] == TILE.ROCK)
									|| (world[i + 1][j] == TILE.WALL || world[i + 1][j] == TILE.ROCK)) {

								if (world[i + 1][j] == TILE.WALL
										|| world[i + 1][j] == TILE.ROCK) {

									mur.add(new Point[] { new Point(i, j),
											leftBorder(i + 1, j) });

								}

								if (world[i][j + 1] == TILE.WALL
										|| world[i][j + 1] == TILE.ROCK) {
			...
			for (int i = 0; i < mur.size(); i++) {
				System.out.println("(" + mur.get(i)[0].x + ","
						+ mur.get(i)[0].y + ")-(" + mur.get(i)[1].x + ","
						+ mur.get(i)[1].y + ")");
			}
		}
