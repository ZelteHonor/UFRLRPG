...
public class Generator {
	...
	/**
	 * G�n�re le monde, sous forme d'un array de l'enum World.TILE
	 * 
	 * @return world array
	 */
	...
	/**
	 * Pr�pare le monde, en settant les variables et vidant les arrays
	 */
	private void clear() {
	...
	/**
	 * G�n�re ROOM_COUNT salles, dont ROOOM_BIG_COUNT sont des "grosses" salles.
	...
	/**
	 * S�lectionne des salles, et les connectes avec des tunnels.
	 */
	private void generateTunnels() {
		...
		while (rooms.size() > 0) {
			...
			/*
			 * Tente de trouver une salle qui est dans la limite acceptable, et
			 * qui est d�j� connect�e � une autre salles
			 */
	...
	/**
	 * G�n�re des grottes, bas�e sur un tableau de bruit
	 */
	...
	private void generatesExits() {
		exitupx = (rooms.get(0).x + 1)
				+ (int) (Math.random() * (rooms.get(0).w - 1));
		exitupy = (rooms.get(0).y + 1)
				+ (int) (Math.random() * (rooms.get(0).h - 1));

		exitdownx = (rooms.get(rooms.size() - 1).x + 1)
				+ (int) (Math.random() * (rooms.get(rooms.size() - 1).w - 1));
		exitdowny = (rooms.get(rooms.size() - 1).y + 1)
				+ (int) (Math.random() * (rooms.get(rooms.size() - 1).h - 1));
	}

	/**
	 * Connecte les groupes de salles, jusqu'� ce qu'elle soit toutes connect�es
	 */
		...
		for (int i = 0; i < World.SIZE; i++)
			for (int j = 0; j < World.SIZE; j++)
				if (tiles[i][j] != World.TILE.WALL
						&& tiles[i][j] != World.TILE.CAVE && map[i][j] == 0)
	...
	/**
	 * Enl�ve les grottes qui ne sont pas accesible des couloirs
	 */
	...
	/**
	 * Enl�ve les tuiles qui ne sont pas visible, et sp�cifie le type de mur
	 */
	private void specify() {
		for (int i = 0; i < World.SIZE; i++)
			for (int j = 0; j < World.SIZE; j++)
				if (tiles[i][j] == World.TILE.WALL)
					if (nextTo(i, j, World.TILE.DONJON)
							|| nextTo(i, j, World.TILE.TUNNEL))
						;
	...
	/**
	 * V�rifie si une tuile type est � proximit� directe de x,y
	 * 
	 * @param x
	 * @param y
	 * @param type
	...
	/**
	 * Fonction r�cursive pour inonder les salles
	 * 
	 * @param x
	 *            de d�part
	 * @param y
	...
	/**
	 * Cr�e un tunnel entre deux salles
	 * 
	 * @param from
	 * @param target
	 */
	...
	/**
	 * Fonction de flou sur l'array des grottes pour attenuer le bruit al�atoire
	 */
	...
	/**
	 * Applique les salles logiques, les tunnels logiques et les grottes dans le
	 * tableau final de monde
	 */
	private void carve() {
		for (int i = 0; i < World.SIZE; i++)
			for (int j = 0; j < World.SIZE; j++)
