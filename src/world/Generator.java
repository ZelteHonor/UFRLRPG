...
public class Generator {
	...
	/**
	 * Gï¿½nï¿½re ROOM_COUNT salles, dont ROOOM_BIG_COUNT sont des "grosses"
	 * salles. S'assure que les salles ne se overlap pas.
	 */
	...
	private void generateTunnels() {
		...
		while (rooms.size() > 0) {
			 ...
			 * qui est dï¿½jï¿½ connectï¿½e ï¿½ une autre salles
			 */
	...
	private void generatesExits() {
		...
		exitdownx = (rooms.get(rooms.size() - 1).x + 1)
				+ (int) (Math.random() * rooms.get(rooms.size() - 1).w);
		exitdowny = (rooms.get(rooms.size() - 1).y + 1)
				+ (int) (Math.random() * rooms.get(rooms.size() - 1).h);
	}

	/**
	 * Connecte les groupes de salles, jusqu'ï¿½ ce qu'elle soit toutes
	 * connectï¿½es
	 */
	...
	private void fill(int x, int y, int[][] map, int val) {
		...
		if (x > 0 && tiles[x - 1][y] != World.TILE.WALL && map[x - 1][y] == 0)
			fill(x - 1, y, map, val);
		if (x < World.SIZE - 1 && tiles[x + 1][y] != World.TILE.WALL
				&& map[x + 1][y] == 0)
			fill(x + 1, y, map, val);
		if (y > 0 && tiles[x][y - 1] != World.TILE.WALL && map[x][y - 1] == 0)
			fill(x, y - 1, map, val);
		if (y > World.SIZE - 1 && tiles[x][y + 1] != World.TILE.WALL
				&& map[x][y + 1] == 0)
			fill(x, y + 1, map, val);
	}
	...
	/**
	 * Fonction de flou sur l'array des grottes pour attenuer le bruit
	 * alï¿½atoire
	 */
